...
const extractPeriodsForDate = (timeTables, targetDateStr) => {
  ...
  timeTables.forEach(classObj => {
    ...
    if (dates[targetDateStr]) {
      dates[targetDateStr].forEach(p => periods.push({ 
        ...p, 
        _date: targetDateStr,
...
const TimetableScreen = ({ navigation }) => {
  ...
  return (
    <SafeAreaView style={styles.safeArea} edges={[]}>
      <CustomHeader title="Timetable" showBack={true} />
              ...
              return (
                <View key={i} style={styles.periodCard}>
                  ...
                  <View style={styles.periodContent}>
                    ...
                    <View style={{ gap: 4 }}>
                      {!period.isBreak && period.className && (
                        <View style={styles.roomRow}>
                          <Icon name="users" size={13} color={colors.textMuted} />
                          <Text style={styles.roomText}>  Class: {period.className} {period.section}</Text>
                        </View>
                      )}
